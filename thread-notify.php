...
if ( $user_ids  ) {
	foreach ( $user_ids as $user_id ) {
		...
		if ( $user ) {
			$to_users[] = $user->nickname.' <'.$user->user_email.'>';
			$user_tags[] = $user->nickname;
		}
	}
}
...
$db->query( $db->prepare("INSERT INTO $db->messages ( hash, thread_id, dt, email, content, message_type ) VALUES ( %s, %d, %s, %s, %s, 'note' )", $hash, $thread_id, $dt, "support+$current_user->user_login@{$email_domain}", "Note sent to {$to_email}:\n\n{$message}" ) );
