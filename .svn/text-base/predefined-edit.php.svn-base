...
<p>Title:<br />
<input type="text" size="100" name="name" value="<?php echo htmlspecialchars($name); ?>" class="title reply replying" /></p>
<p>Tag (optional):<br />
<input type="text" size="100" name="tags" value="<?php echo htmlspecialchars($tags); ?>" class="title reply replying" /></p>
<p>Message:<br />
...
<ul>
<?php
$rows = $db->get_results("SELECT * FROM $db->predefined_messages ORDER BY id ASC");
foreach ( $rows as $row ) {
	?>
	<li><a href="?id=<?php echo intval($row->id); ?>"><?php echo htmlspecialchars($row->name); ?></a></li>
	<?php
}
?>
</ul>
