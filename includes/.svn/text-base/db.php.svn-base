...
class bbdb {
	...
	function db_connect( $query = 'SELECT' ) {
		...
		if ( defined('USER_BBDB_NAME') && ( $table == $this->users || $table == $this->usermeta ) ) { // global user tables
			...
			$server->database = constant('DB_NAME');
			$server->user = constant('DB_USER');
			$server->pass = constant('DB_PASSWORD');
			$server->host = constant('DB_HOST');
		}
...
//			$this->handle_error_connecting( $dbh, array( "db" => $db ) );
	...
	function query($query) {
		...
		$delta = $this->timer_stop();
		$this->query_time += $delta;
		if (SAVEQUERIES)
			$this->queries[] = array( $query . ' server:' . $current_connection, $delta );
...
$db = new bbdb(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST);
